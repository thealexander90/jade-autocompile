(function() {
    ...
    function getOptions(file, options) {
        ...
        var extracted = /^\/\/(-)?(?!-)(.|\n[ \t])*/.exec(file);
        var params = extracted ? extracted[0].substring(extracted[0].indexOf("{") - 1) : undefined;
        ...
        if (params) {
            ...
            for ( prop in params) {
                ...
                if (name == 'self' || name == 'debug' || name == 'compileDebug' || name == 'cache' || name == 'pretty') {
                    options[name] = (value.toString().toLowerCase() == 'true');
                }
                ...
                if (_this.fileExt == '.jade') {
                    ...
                    _this.compile(editor.getURI()).then(function(err, _jade, options) {
                        ...
                        if (err) {
                            ...
                            atom.notifications.addWarning(discomposed[0] + '\n' + discomposed[discomposed.length - 1], {
                                dismissable: true,
                                detail: discomposed.slice(1, discomposed.length - 2).join('\n')
                            });
                            if (_this.activeEditor)
                                _this.activeEditor.jade_compiled = false;
                            if (_this.icon)
                        ...
                        _this.saveJade(options.filename, _jade, options.output, function() {
            ...
            fs.readFile(file, { encoding: 'utf-8' },
                ...
                if (!options.output) {
                    ...
                    if (!options.main) {
                        ...
                        var curr_dir = path.dirname(file);

                        // Checking for type of property to avoid problems with single page without []
                        if (options.main.length > 1 && typeof options.main == "object") {

                            for (var el of options.main) {
                                options.current_file = curr_dir + "\\" + el;
                                _this.compile(options.current_file)
                            }

                        } else {
                            options.current_file = curr_dir + "\\" + options.main;
                            _this.compile(options.current_file)
                        }
        ...
        saveJade: function saveJade(filePath, _jade, name, cb) {
            ...
            fs.writeFile(result, _jade, function writeFile(err) {
                if (err) {
                    ...
                    atom.notifications.addSuccess(`File ${originalName}.${originalExt} compilato con successo`, {
                        dismissable: false
                    });
